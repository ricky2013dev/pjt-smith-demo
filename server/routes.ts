...
export async function registerRoutes(
  ...
  app.post("/api/patients/:id/insurance/:insuranceId/decrypt", requireAuth, async (req, res) => {
    try {
      ...
      console.error("Error decrypting insurance data:", error);
  ...
  app.post("/api/patients/:id/insurance-card-ocr",
    ...
    async (req, res) => {
      try {
        ...
        console.log(`Processing insurance card OCR for patient ${id}`);
  ...
  app.get("/api/transactions", requireAuth, async (req, res) => {
    try {
      console.log('[API] GET /api/transactions - User:', (req.session as any)?.userId);
  ...
  app.post("/api/transactions", requireAuth, async (req, res) => {
    try {
      ...
      console.log('[API] POST /api/transactions - User:', userId);
      console.log('[API] Transaction data received:', JSON.stringify(transactionData, null, 2));
      ...
      console.log('[API] Inserting transaction with data:', JSON.stringify(txnData, null, 2));
  ...
  app.post("/api/coverage-by-code/:patientId", requireAuth, async (req, res) => {
    try {
      ...
      console.error("Error saving coverage by code data:", error.message);
      res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  app.get("/api/coverage-by-code/:patientId", requireAuth, async (req, res) => {
    try {
      ...
      console.error("Error fetching coverage by code data:", error.message);
      res.status(500).json({
        success: false,
        error: error.message
      });
