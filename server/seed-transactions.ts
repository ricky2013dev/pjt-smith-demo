...
export async function seedTransactions() {
  try {
    ...
    if (!existingPatient) {
      console.log('Creating patient P000001...');
      ...
      console.log('Patient P000001 created successfully');
    } else {
      console.log('Patient P000001 already exists');
    }
    ...
    console.log('Cleared existing transactions for patient P000001');

    // Insert sample transactions
    for (const txn of sampleTransactions) {
      const [insertedTxn] = await db.insert(transactions).values(txn).returning();
      console.log(`Inserted transaction: ${insertedTxn.requestId}`);
