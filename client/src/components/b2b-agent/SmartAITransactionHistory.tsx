...
const SmartAITransactionHistory: React.FC<SmartAITransactionHistoryProps> = ({ patientId, refreshTrigger }) => {
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        console.error('[TransactionHistory] Error fetching current user:', error);
      }
    };
    fetchCurrentUser();
  }, []);
  ...
  const fetchTransactions = async () => {
    try {
      ...
      console.log('[TransactionHistory] Data Mode:', dataMode ? 'ON' : 'OFF');

      if (!dataMode) {
        // Data mode is OFF - use mock data
        console.log('[TransactionHistory] Using mock data (Data Mode OFF)');
      ...
      console.log('[TransactionHistory] Fetching from database (Data Mode ON) for patient:', patientId);
      ...
      if (!response.ok) {
        ...
        console.error('[TransactionHistory] Error response:', errorText);
      ...
      if (data.success && data.transactions) {
        console.log('[TransactionHistory] Total transactions:', data.transactions.length);
        ...
        console.log('[TransactionHistory] Filtered transactions for patient:', filteredTransactions.length);
        setTransactions(filteredTransactions);
      } else {
        console.log('[TransactionHistory] No transactions in response');
        setTransactions([]);
      }
    } catch (error) {
