...
const PatientDetail: React.FC<PatientDetailProps> = ({
  ...
  const handleSave = async () => {
    if (onSavePatient) {
      ...
      console.log("Saving changes (mockup mode):", {
        name: { given: [editedFirstName, editedMiddleName].filter(Boolean), family: editedLastName },
        birthDate: editedBirthDate,
        gender: editedGender,
        active: editedActive,
        insurance: editedInsurance,
          ...
          <TabContent>
            ...
            {(isEditing && editedInsurance.length > 0) || (!isEditing && (patient as any).insurance?.length > 0) ? (
              ...
              <div className="border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-8 text-center">
                {(() => {
                  console.log('RENDERING: Add Insurance Button Branch - isEditing:', isEditing);
                  return null;
                })()}
              ...
              <div className="border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-12 text-center">
                {(() => {
                  console.log('RENDERING: No Edit Mode - Non-Edit Branch - isEditing:', isEditing);
                  return null;
                })()}
