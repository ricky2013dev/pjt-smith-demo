...
Plan Renewal:           January 1st`;
  ...
  const saveTransactionHistory = async (startTime: Date, endTime: Date) => {
    ...
    if (!dataMode || !stediTest) {
      console.log('Transaction not saved - Data Mode or STEDI Test is off');
      return;
    }
  ...
  const saveCoverageByCodeData = async () => {
    ...
    // Only save if data mode is on and STEDI test is on
    if (!dataMode || !stediTest) {
      console.log('Coverage data not saved - Data Mode or STEDI Test is off');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        console.log('Coverage by code data saved successfully');
      } else {
        const error = await response.json();
