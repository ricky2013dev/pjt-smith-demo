...
import { decryptInsuranceField } from '@/services/sensitiveDataService';

interface InsuranceSensitiveDataFieldProps {
  patientId: string;
  insuranceId: string;
  fieldName: string;
  maskedValue: string;
...
const InsuranceSensitiveDataField: React.FC<InsuranceSensitiveDataFieldProps> = ({
  patientId,
  insuranceId,
  fieldName,
  maskedValue,
  ...
  const handleViewClick = async () => {
    ...
    try {
      const value = await decryptInsuranceField(patientId, insuranceId, fieldName);
...
export default InsuranceSensitiveDataField;
